...
def parametric_double_ml_cate(train_pred_y, test):
    ...
    final_model_cate = smf.ols(formula='sales_res ~ price_res * (temp + C(weekday) + cost)', data=train_pred_y).fit()
...
def orthogonalize_treatment_and_outcome(train):
    ...
    debias_m = LGBMRegressor(max_depth=3)
    denoise_m = LGBMRegressor(max_depth=3)
    train_pred_nonparam = train.assign(price_res =  train[T] - cross_val_predict(debias_m, train[X], train[T], cv=5),
