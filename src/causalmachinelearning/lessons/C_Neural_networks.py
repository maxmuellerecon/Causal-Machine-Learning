...
def build_model_using_sequential():
    """Generate NN, First layer has 160 neurons, second layer has 480 neurons, third
    layer has 256 neurons, output layer has 1 neuron where, the 3 layers use relu
    activation function and the output layer uses linear activation function.
...
def compile_and_train_model(model, x_train_scaled, y_train):
    ...
    model.compile(
        loss=msle, optimizer=Adam(learning_rate=learning_rate), metrics=[msle]
    )
    # train the model
    history = model.fit(
...
def assess_performance(x_test_pred, y_test):
    """Assess the performance of the model, by calculating the root mean squared
    error."""
    _fail_if_not_dataframe(x_test_pred)
    _fail_if_not_dataframe(y_test)

    rmse = math.sqrt(
        sum((x_test_pred["prediction"] - y_test["median_house_value"]) ** 2)
        / len(x_test_pred)
    )
    return rmse
