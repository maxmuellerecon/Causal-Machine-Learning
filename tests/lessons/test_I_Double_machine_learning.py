...
from causalmachinelearning.lessons.I_Double_machine_learning import (
    plot_pattern,
    verify_fwl_theorem,
    debias_treatment,
    denoise_outcome,
    comparison_models,
    parametric_double_ml_cate,
    non_parametric_double_ml_cate,
...
def test_verify_fwl_theorem():
    # Prepare test data
    df = pd.DataFrame(
        {
            "price": [1.99, 2.99, 3.99, 4.99, 5.99],
    ...
    table2 = verify_fwl_theorem(df)
    assert not isinstance(table2, str)
...
def test_denoise_outcome_shape_output():
    train = pd.read_csv(SRC / "data" / "ice_cream_sales.csv")
    train_pred = pd.read_csv(BLD / "python" / "Lesson_I" / "data" / "train_pred.csv")
    train_pred_y = denoise_outcome(train, train_pred)
...
def test_comparison_models_type_input_pred():
    train = pd.read_csv(SRC / "data" / "ice_cream_sales.csv")
    train_pred_y = pd.read_csv(
        BLD / "python" / "Lesson_I" / "data" / "train_pred_y.csv"
    )
...
def test_non_parametric_double_ml_cate_type_output():
    train_pred_nonparam = pd.read_csv(
        BLD / "python" / "Lesson_I" / "data" / "train_pred_nonparam.csv"
    )
    test = pd.read_csv(SRC / "data" / "ice_cream_sales_rnd.csv")
